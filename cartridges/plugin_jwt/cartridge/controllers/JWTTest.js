...
server.get('RSA', function (req, res, next) {
    ...
    var inlineKeys = getInlineRsaKeys();
    var privateKey = inlineKeys.privateKey;
    var publicKey = inlineKeys.publicKey;
    ...
    var responseData = { decodedToken: decodedToken, verified: verified };
    if (includeToken) {
...
server.get('Matrix', function (req, res, next) {
    if (!isJWTTestEnabled()) {
        res.setStatusCode(404);
        res.json({ error: 'Not Found' });
        return next();
    ...
    var includeTokens = String(req.querystring.includeTokens || '') === 'true';
    var secret = 'my_secret';
    var inlineKeys = getInlineRsaKeys();

    var algorithms = [
        { alg: 'HS256', keyType: 'hmac' },
        { alg: 'HS384', keyType: 'hmac' },
        { alg: 'HS512', keyType: 'hmac' },
        { alg: 'RS256', keyType: 'rsa' },
        { alg: 'RS384', keyType: 'rsa' },
        { alg: 'RS512', keyType: 'rsa' },
        { alg: 'PS256', keyType: 'rsa' },
        { alg: 'PS384', keyType: 'rsa' },
        { alg: 'PS512', keyType: 'rsa' }
    ];

    var results = [];
    var allVerified = true;
    for (var i = 0; i < algorithms.length; i++) {
        var item = algorithms[i];
        var result = { algorithm: item.alg };
        ...
        try {
            ...
            var signOptions = {
                privateKeyOrSecret: item.keyType === 'hmac' ? secret : inlineKeys.privateKey,
                algorithm: item.alg,
                kid: 'kid-' + item.alg
            };
            ...
            var verifyOptions = {
                publicKeyOrSecret: item.keyType === 'hmac' ? secret : inlineKeys.publicKey,
